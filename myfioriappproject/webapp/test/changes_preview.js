        ...
        return new Promise(function (resolve, reject) {
          ...
          $.ajax({
            ...
            .then(function (oCachebusterContent) {
              ...
              var aChangeFilesPaths = Object.keys(oCachebusterContent).filter(
                function (sPath) {
              ...
              $.each(aChangeFilesPaths, function (index, sFilePath) {
                ...
                if (sFilePath.indexOf("changes") === 0) {
                  ...
                  if (!isValidHost)
                    reject(
                      console.log("cannot load flex changes: invalid host")
                    );
                  aPromises.push(
                    $.ajax({
              ...
              return Promise.all(aPromises).then(function (aChanges) {
                return new Promise(function (resolve, reject) {
                      ...
                      .then(function (sChangesFolderContent) {
                        ...
                        while (result !== null) {
                          if (!isValidHost)
                            reject(
                              console.log(
                                "cannot load flex changes: invalid host"
                              )
                            );
                          aPromises.push(
                            $.ajax({
                  ...
                  if (aChangePromises.length > 0) {
                    return Promise.all(aChangePromises).then(function (
                      aUpdatedChanges
                    ) {
                      ...
                      aProcessedChanges.sort(function (change1, change2) {
                        return (
                          new Date(change1.creation) -
                          new Date(change2.creation)
                        );
                      });
                    ...
                    aProcessedChanges.sort(function (change1, change2) {
                      return (
                        new Date(change1.creation) - new Date(change2.creation)
                      );
                    });
                    oResult.changes = aProcessedChanges;
                    var oLrepChange = {
